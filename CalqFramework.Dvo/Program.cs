...
internal class Program
{
    ...
    ///     Thrown when any invoked shell command (via <c>RUN</c> or <c>CMD</c>) fails and cannot be recovered by stashing or
    ///     retry logic.
    /// </exception>
    ...
    ///     <list type="number">
    ///         <item>
    ///             <description>Validate inputs; delegate to <c>dotnet</c> CLI on missing parameters.</description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 Derive a kebab-case and snake-case short name from the portion of <c>projectFullName</c> after
    ///                 the first dot.
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <description>Create and switch into a directory named by the kebab-case short name.</description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 Depending on <c>type</c>, invoke <c>dotnet new</c> to scaffold console apps, class libraries,
    ///                 unit tests, and solutions.
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 For class libraries, apply GitHub source link package and update the .csproj&apos;s
    ///                 RootNamespace, PackageId, and Version elements.
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <description>
    ///                 If <c>organization</c> is specified, fetch and install .gitignore, CI templates, LICENSE,
    ///                 README, and push an initial commit to GitHub.
    ///             </description>
    ///         </item>
    ///     </list>
    /// </remarks>
    public void Init(string type, [CliName("projectFullName")] [CliName("n")] string projectFullName,
        string? organization = null)
    {
        ...
        var projectKebabName = projectNameWithoutRootNamespace;
        ...
        var projectSnakeName = projectNameWithoutRootNamespace;
        ...
        switch (type)
        {
            ...
            case "classlib":
                ...
                (xmlDoc.SelectSingleNode("/Project/PropertyGroup/RootNamespace") ??
                 xmlDoc.SelectSingleNode("/Project/PropertyGroup")!.AppendChild(xmlDoc.CreateElement("RootNamespace"))!)
                    .InnerText = projectFullName;
                (xmlDoc.SelectSingleNode("/Project/PropertyGroup/PackageId") ??
                 xmlDoc.SelectSingleNode("/Project/PropertyGroup")!.AppendChild(xmlDoc.CreateElement("PackageId"))!)
                    .InnerText = projectFullName;
                (xmlDoc.SelectSingleNode("/Project/PropertyGroup/Version") ??
                 xmlDoc.SelectSingleNode("/Project/PropertyGroup")!.AppendChild(xmlDoc.CreateElement("Version"))!)
                    .InnerText = "0.0.0";
                ...
                {
                    var gitignoreText = new HttpClient()
                        .GetStringAsync(
                            "https://www.toptal.com/developers/gitignore/api/linux,macos,windows,dotnetcore,monodevelop,visualstudio,visualstudiocode,rider")
                        .Result;
                    File.WriteAllText(Path.Combine(PWD, ".gitignore"), gitignoreText);

                    var workflowsDir = Path.Combine(PWD, ".github", "workflows");
                    ...
                    var sourceDir = Path.Combine(cloneDir, "workflow-templates");
                    foreach (var sourcePath in Directory.GetFiles(sourceDir))
                    {
                        var fileName = Path.GetFileName(sourcePath);
                        var destPath = Path.Combine(workflowsDir, fileName);
                        var contents = File.ReadAllText(sourcePath).Replace("$default-branch", "main");
        ...
        if (!string.IsNullOrEmpty(organization))
        {
            ...
            RUN(
                $"gh repo create {organization}/{projectKebabName} --private --source=. --remote=origin --disable-wiki");
    ...
    public void Issue(string titleOrNumber)
    {
        ...
        var createOutput = CMD($"gh issue create --title \"{titleOrNumber}\" --body \"\"");
    ...
    public void Pr()
    {
        ...
        {
            var issueNumber = branchName.Replace("issues/", string.Empty);
            var issueInfo = CMD($"gh issue view {issueNumber} --json title");
            var issueJson = JsonSerializer.Deserialize<JsonElement>(issueInfo);
            var issueTitle = issueJson.GetProperty("title").GetString()!;
    ...
    public void Merge()
    {
        ...
        var issueNumber = branchName.StartsWith("issues/")
